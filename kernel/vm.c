...
void vmprint_(pagetable_t pagetable, int depth){
...
// Lab3 , copy user pagetable to kernel pagetable
void copytokpt(pagetable_t pagetable, pagetable_t kpagetable, uint64 oldsz, uint64 newsz)
{ 
  ...
  for (a = oldsz; a < newsz; a += PGSIZE)
  {
	if ((pte_from = walk(pagetable, a, 0)) == 0)
	  panic("copytokpt: pte should exist");
